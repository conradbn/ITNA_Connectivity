...
dsets_rh = dir('LitCoord_*_std.141_rh.inflated.4mm_diam.niml.dset');

% dsets_lh = dir('LitCoord_Letter_*_std.141_lh.inflated.4mm_diam.niml.dset');
% dsets_rh = dir('LitCoord_Letter_*_std.141_rh.inflated.4mm_diam.niml.dset');
%% LH
mesh = '141';
hemi = 'lh';
dsets = dsets_lh;
    ...
    for ii = 1:numel(dsets)
        ...
        if contains(n,'Letter')
            ...
            str = ['DriveSuma -com surf_cont -load_dset ' n ...
                ' -switch_cmap blue_monochrome -I_sb 1 -I_range 1 -Dim 1 -1_only n'];eval(addstr); % Dim 1
