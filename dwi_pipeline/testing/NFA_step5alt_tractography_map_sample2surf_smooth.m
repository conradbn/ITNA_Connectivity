...
for ii = 1%:numel(sub_dirs)
    ...
    for jj = 1:numel(rois)
        ...
        if strcmp(r(1),'.') % Skip files starting with .
            ...
            tck = ['tracks_ss3t_' sub '_combined_nofdscale_' r];
            unix(['tckmap -force -template brain.finalsurfs_al2dwi.nii.gz -ends_only -backtrack -tck_weights_in'...
                ' tracks_sift2_weights_ss3t_' sub '_combined_nofdscale_' r '.txt'...
                ' ' tck '.tck'...
                ' ' tck '.TDI_ends.nii']);
            % Scale by the subject-specific proportionality coefficient
            mu = load(['tracks_sift2_weights_ss3t_' sub '_combined_nofdscale_prop_coeff.txt']);
