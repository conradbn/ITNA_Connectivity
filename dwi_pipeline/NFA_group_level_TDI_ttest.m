...
for ii = 1:numel(hemi)
    ...
    if size(d,1) == 2
        unix(['gen_group_command.py -command ' command...
            ...
            ' -options \'...
            ' -paired ' covars_str...
            ' -resid resid_' cmdlbl '_3dttest++.' h '.niml.dset'... 
            ' -tempdir ' tempdir]);
        %     ' -mask ' maskgroup ...
        %     ' -Clustsim '.
        %     ' -ETAC -ETAC_blur ' blur...
...
%% Post process the outputs    
    % Add mean back to residuals to yield data with covariates "regressed
    % out" (a la
    % https://afni.nimh.nih.gov/afni/community/board/read.php?1,147682,147722#msg-147722)
    unix(['3dcalc -a resid_' cmdlbl '_3dttest++.' h '.niml.dset'...
        ' -b ' cmdlbl '_3dttest++.' h '.niml.dset[0]'... 
        ' -prefix resid_from_cov_' cmdlbl '_3dttest++.' h '.niml.dset'...
        ' -expr "a+b"']);
    ...
    unix(['3dcalc -a ' cmdlbl '_3dttest++.' h '.niml.dset'...
        ...
        ' -expr "' mask_expr '"']);
    % Create masked residual result
    unix(['3dcalc -a resid_' cmdlbl '_3dttest++.' h '.niml.dset'...
        ' -b ' maskgroup '[1]'...
        ' -prefix resid_' cmdlbl '_3dttest++.' h '.mask.niml.dset'...
        ' -expr "' mask_expr '"']);
    % Create masked residual result
    unix(['3dcalc -a resid_from_cov_' cmdlbl '_3dttest++.' h '.niml.dset'...
        ' -b ' maskgroup '[1]'...
        ' -prefix resid_from_cov_' cmdlbl '_3dttest++.' h '.mask.niml.dset'...
        ' -expr "' mask_expr '"']);
