...
%% ROIs seperated by condition

% LH - Digit
roi_files = dir('/Volumes/NBL_Projects/Price_NFA/NFA_fMRI/ProcessedData/*_proc/*.freesurfer/SUMA/std.141.lh.PollackandPrice19_Dp-Da.MNI152.votc.inflated.14mm_diam.1.1D');
nodes = [];
for ii = 1:numel(roi_files)
    fid = fopen([roi_files(ii).folder '/' roi_files(ii).name]);
    cdata = textscan(fid,'%f%f','delimiter',',', 'HeaderLines',2);
...
nodes_unique = unique(nodes);
dlmwrite('lh_rois_Digit.1D',[nodes_unique,ones(size(nodes_unique))],'Delimiter','\t','precision',10)
unix(['ConvertDset -o_niml -pad_to_node ld141'...
      ' -input lh_rois_Digit.1D -prefix lh_rois_Digit -node_index_1D lh_rois_Digit.1D[0]']);

unix('3dcalc -prefix lh_final_group_mask_Digit.niml.dset -a group_50perc_union.lh.niml.dset -b lh_rois_Digit.niml.dset -expr "a*not(b)"');


% RH - Digit
roi_files = dir('/Volumes/NBL_Projects/Price_NFA/NFA_fMRI/ProcessedData/*_proc/*.freesurfer/SUMA/std.141.rh.PollackandPrice19_Dp-Da.MNI152.votc.inflated.14mm_diam.1.1D');
nodes = [];
for ii = 1:numel(roi_files)
    fid = fopen([roi_files(ii).folder '/' roi_files(ii).name]);
    cdata = textscan(fid,'%f%f','delimiter',',', 'HeaderLines',2);
...
nodes_unique = unique(nodes);
dlmwrite('rh_rois_Digit.1D',[nodes_unique,ones(size(nodes_unique))],'Delimiter','\t','precision',10)
unix(['ConvertDset -o_niml -pad_to_node ld141'...
      ' -input rh_rois_Digit.1D -prefix rh_rois_Digit -node_index_1D rh_rois_Digit.1D[0]']);

unix('3dcalc -prefix rh_final_group_mask_Digit.niml.dset -a group_50perc_union.rh.niml.dset -b rh_rois_Digit.niml.dset -expr "a*not(b)"');

% LH - Letter
roi_files = dir('/Volumes/NBL_Projects/Price_NFA/NFA_fMRI/ProcessedData/*_proc/*.freesurfer/SUMA/std.141.lh.PollackandPrice19_Lp-La.MNI152.votc.inflated.14mm_diam.1.1D');
nodes = [];
for ii = 1:numel(roi_files)
    fid = fopen([roi_files(ii).folder '/' roi_files(ii).name]);
    cdata = textscan(fid,'%f%f','delimiter',',', 'HeaderLines',2);
...
nodes_unique = unique(nodes);
dlmwrite('lh_rois_Letter.1D',[nodes_unique,ones(size(nodes_unique))],'Delimiter','\t','precision',10)
unix(['ConvertDset -o_niml -pad_to_node ld141'...
      ' -input lh_rois_Letter.1D -prefix lh_rois_Letter -node_index_1D lh_rois_Letter.1D[0]']);

unix('3dcalc -prefix lh_final_group_mask_Letter.niml.dset -a group_50perc_union.lh.niml.dset -b lh_rois_Letter.niml.dset -expr "a*not(b)"');


% RH - Letter
roi_files = dir('/Volumes/NBL_Projects/Price_NFA/NFA_fMRI/ProcessedData/*_proc/*.freesurfer/SUMA/std.141.rh.PollackandPrice19_Lp-La.MNI152.votc.inflated.14mm_diam.1.1D');
nodes = [];
for ii = 1:numel(roi_files)
    fid = fopen([roi_files(ii).folder '/' roi_files(ii).name]);
    cdata = textscan(fid,'%f%f','delimiter',',', 'HeaderLines',2);
...
nodes_unique = unique(nodes);
dlmwrite('rh_rois_Letter.1D',[nodes_unique,ones(size(nodes_unique))],'Delimiter','\t','precision',10)
unix(['ConvertDset -o_niml -pad_to_node ld141'...
      ' -input rh_rois_Letter.1D -prefix rh_rois_Letter -node_index_1D rh_rois_Letter.1D[0]']);

unix('3dcalc -prefix rh_final_group_mask_Letter.niml.dset -a group_50perc_union.rh.niml.dset -b rh_rois_Letter.niml.dset -expr "a*not(b)"');
