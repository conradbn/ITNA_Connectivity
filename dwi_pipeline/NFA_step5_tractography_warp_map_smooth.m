...
cd(start_dir);
sub_dirs = dir('Price_*');
local_dir = '/Users/nbl_imac2/Desktop/Price_NFA_Tractography_MNI152';%'/Users/benconrad/Desktop/Price_NFA_Tractography';%'/Users/nbl_imac2/Desktop/Price_NFA_Tractography';
...
for ii = 1:numel(sub_dirs)
    ...
    for jj = 1:numel(rois)
        ...
        unix(['tckmap -force -template ' template ' -ends_only -backtrack -tck_weights_in'...
            ' tracks_sift2_weights_ss3t_' sub '_combined_' r '.txt'...
            ' tracks_ss3t_' sub '_combined_' r '.MNI.tck'...
            ' tracks_ss3t_' sub '_combined_' r '.MNI.TDI_ends.nii']);
        % Smooth the TDI maps
        kernel = {'2' '4' '6' '8'};
        for kk = 1:numel(kernel)
            unix(['mrfilter -force -fwhm ' kernel{kk}...
                ' tracks_ss3t_' sub '_combined_' r '.MNI.TDI.nii'...
                ' smooth'...
                ' tracks_ss3t_' sub '_combined_' r '.MNI.TDI.smooth' kernel{kk} 'mm.nii']);
            unix(['mrfilter -force -fwhm ' kernel{kk}...
                ' tracks_ss3t_' sub '_combined_' r '.MNI.TDI_ends.nii'...
                ' smooth'...
                ' tracks_ss3t_' sub '_combined_' r '.MNI.TDI_ends.smooth' kernel{kk} 'mm.nii']);
        end
    end
