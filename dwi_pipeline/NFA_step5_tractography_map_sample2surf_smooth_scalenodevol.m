...
%             in =  [tck '.TDI_ends.norm.nii'];
%             out = [tck '.TDI_ends.norm.al2anat.nii'];
...
%         ' -prefix aparc.a2009s+aseg_REN_gmrois.al2dwi.nii.gz']);
    ...
    for jj = 1:numel(rois)
        ...
        if strcmp(r(1),'.') % Skip files starting with .
            continue
        elseif strcmp(r,'Dp-Da.rh') || strcmp(r,'Lp-La.rh')
            continue
        end
        ...
        % Extract the connectivity weights - controlling for volume
        unix(['tck2connectome -scale_invnodevol -vector ' tck '.tck -tck_weights_in ' tck_weights...
            ' aparc.a2009s+aseg_REN_gmrois.al2dwi.nii.gz '...
            tck '.fingerprint.scalevol.csv']);
        
        % Scale by the subject-specific proportionality coefficient (mu)
        f = readmatrix([tck '.fingerprint.scalevol.csv'],'NumHeaderLines',1);
        f = f.*mu;
        writematrix(f,[tck '.fingerprint.scalevol.norm.csv']);
