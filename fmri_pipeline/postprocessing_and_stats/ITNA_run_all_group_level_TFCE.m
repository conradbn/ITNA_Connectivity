...
% contrast, then the group mask, then the two datasets for comparison (N
% contasts x 3 cell array). Each dataset file is a 4D dataset with every
% subject's connectivity map catenated into one file.
...
input_strings = {
    ...
    'PairedTest_FC_Lp_DigLH_vs_LetLH','lh','ld60',...
    'GroupMask_DigLH_LetLH_ld60.niml.dset',...
    ...
    'PairedTest_FC_Dp_DigLH_vs_DigRH_on_LHsurf','lh','ld60',...
    ...
    'GroupMask_DigLH_DigRH_on_RHsurf_ld60.niml.dset'...
        'AllSubs_Dp-Da.lh.beta_series_corr.lh.Zmap.Dp_MAP2CON.niml.dset',...
        'AllSubs_Dp-Da_math.rh.beta_series_corr.rh.Zmap.Dp.niml.dset';
...
cd(data_dir)
for ii = 12%7:8%:size(input_strings,1)
...
% Also get the uncorrected z-scores for verification 
